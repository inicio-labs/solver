...
fn match_self_bottleneck() {
    let mut order_a = make_order(0, eth(), usdc(), 100, 50);
    let mut order_b = make_order(1, usdc(), eth(), 200, 200);
...
fn match_other_bottleneck() {
    let mut order_a = make_order(0, eth(), usdc(), 1000, 500);
    let mut order_b = make_order(1, usdc(), eth(), 50, 30);
...
#[test]
fn profitable_check() {
    let order_a = make_order(0, eth(), usdc(), 2000, 1);
    let good = make_order(1, usdc(), eth(), 1, 1600);
    let even = make_order(2, usdc(), eth(), 1, 2000);
    let bad = make_order(3, usdc(), eth(), 1, 2100);
...
fn order_fill_and_remaining() {
    let mut order = make_order(0, eth(), usdc(), 2000, 10);
