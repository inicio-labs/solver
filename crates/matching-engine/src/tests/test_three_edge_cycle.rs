...
fn no_profitable_cycle() {
    ...
    book.add_user_order(eth(), usdc(), 10, 19000);
    book.add_user_order(usdc(), sol(), 19000, 120);
    book.add_user_order(sol(), eth(), 120, 10);
...
fn at_least_one_order_exhausted() {
    ...
    let initial_count = book.orders.len();
    let (filled, cycles) = run_three_edge_cycle(&mut book);
    assert!(cycles > 0);

    let exhausted = (0..initial_count)
        .filter(|&i| filled.contains(&(i as u32)) && book.orders[i].is_completely_filled())
        .count();
...
fn asset_conservation() {
    ...
    let id_ab = book.add_user_order(eth(), usdc(), 10, 16000).unwrap();
    // USDC→SOL: offers 16000 USDC, requests 80 SOL
    let id_bc = book.add_user_order(usdc(), sol(), 16000, 80).unwrap();
    // SOL→ETH: offers 80 SOL, requests 5 ETH
    let id_ca = book.add_user_order(sol(), eth(), 80, 5).unwrap();
    ...
    // Actually offered_for uses total ratio, but we need the actual released amount.
    // Since fill() returns released, and we can't capture it here, check via remaining.
    // released = offered - offered_remaining (approximately)
...
fn bottleneck_on_ab() {
    ...
    book.add_user_order(usdc(), sol(), 100000, 500);
    // Large CA: 500 SOL ($75k)
    book.add_user_order(sol(), eth(), 500, 30);

    let (_, cycles) = run_three_edge_cycle(&mut book);
    assert!(cycles > 0);
    // AB should be fully consumed (it was the bottleneck)
    assert!(book.orders[0].is_completely_filled(), "AB (bottleneck) should be exhausted");
    // Others should be partially filled
    assert!(book.orders[1].is_active(), "BC should be partially filled");
    assert!(book.orders[2].is_active(), "CA should be partially filled");
}
...
fn bottleneck_on_bc() {
    ...
    book.add_user_order(usdc(), sol(), 3000, 10);
    // Large CA: 100 SOL ($15k), requests 5 ETH ($10k)
    book.add_user_order(sol(), eth(), 100, 5);

    let (_, cycles) = run_three_edge_cycle(&mut book);
    assert!(cycles > 0, "should execute");
    assert!(book.orders[1].is_completely_filled(), "BC (bottleneck) should be exhausted");
    assert!(book.orders[0].is_active(), "AB should be partially filled");
}
...
fn bottleneck_on_ca() {
    ...
    book.add_user_order(usdc(), sol(), 160000, 800);
    // Tiny CA: 20 SOL ($3k) — smallest
    book.add_user_order(sol(), eth(), 20, 1);

    let (_, cycles) = run_three_edge_cycle(&mut book);
    assert!(cycles > 0);
    assert!(book.orders[2].is_completely_filled(), "CA (bottleneck) should be exhausted");
    assert!(book.orders[0].is_active(), "AB should be partially filled");
}
...
fn multiple_triangles_highest_surplus_first() {
    ...
    book.add_user_order(eth(), usdc(), 10, 16000);
    book.add_user_order(usdc(), btc(), 16000, 1);
    book.add_user_order(btc(), eth(), 1, 5);
...
fn sequential_cycles_with_order_promotion() {
    ...
    // Two ETH→USDC orders
    book.add_user_order(eth(), usdc(), 10, 16000);
    book.add_user_order(eth(), usdc(), 10, 18000);
    // Two USDC→SOL orders
    book.add_user_order(usdc(), sol(), 16000, 80);
    book.add_user_order(usdc(), sol(), 18000, 90);
    // Two SOL→ETH orders
    book.add_user_order(sol(), eth(), 80, 5);
    book.add_user_order(sol(), eth(), 90, 5);
...
fn stale_order_promotes_next_best() {
    ...
    book.add_user_order(eth(), usdc(), 10, 18000);   // less generous
    book.add_user_order(usdc(), sol(), 16000, 80);
    book.add_user_order(sol(), eth(), 80, 5);
    book.add_user_order(usdc(), sol(), 18000, 90);
    book.add_user_order(sol(), eth(), 90, 5);
...
fn tiny_amounts_no_panic() {
    ...
    book.add_user_order(eth(), usdc(), 1, 1);
    book.add_user_order(usdc(), sol(), 1, 1);
    book.add_user_order(sol(), eth(), 1, 1);
...
fn large_amounts_no_overflow() {
    ...
    book.add_user_order(eth(), usdc(), large, large / 2);
    book.add_user_order(usdc(), sol(), large / 2, large / 4);
    book.add_user_order(sol(), eth(), large / 4, large / 8);
...
fn phase1_then_phase2() {
    ...
    book.add_user_order(eth(), usdc(), 5, 8000);
    book.add_user_order(usdc(), eth(), 8000, 5);
...
fn oracle_rejects_one_leg() {
    ...
    let rejected = book.add_user_order(sol(), eth(), 5, 1);
    assert!(rejected.is_none(), "oracle should reject unprofitable order");
...
fn four_tokens_two_triangles() {
    ...
    book.add_user_order(btc(), eth(), 1, 20);         // 1 BTC ($60k) for 20 ETH ($40k)
    book.add_user_order(eth(), sol(), 20, 1600);       // 20 ETH ($40k) for 1600 SOL ($24k) — needs separate SOL→BTC
    book.add_user_order(sol(), btc(), 1600, 1);        // 1600 SOL ($240k) for 1 BTC ($60k)
...
fn five_tokens_stress() {
    ...
    book.add_user_order(usdc(), sol(), 16000, 80);
    book.add_user_order(sol(), eth(), 80, 5);
    book.add_user_order(btc(), usdc(), 1, 48000);
    book.add_user_order(usdc(), matic(), 48000, 800);
    book.add_user_order(matic(), btc(), 800, 1);
