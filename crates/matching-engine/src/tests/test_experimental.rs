...
fn check_invariants(engine: &MatchingEngine<SimpleMapFeed>, batch: &SettlementBatch, label: &str) {
    ...
    for (i, order) in book.orders.iter().enumerate() {
        assert!(
            ...
            label, i, order.requested_filled(), order.requested
        );
        assert!(
    ...
    for &oid in &batch.filled_orders {
        let order = &book.orders[oid as usize];
        assert!(
...
fn fuzz_rate_one() {
    ...
    for trial in 0..50 {
        ...
        for _ in 0..n {
            ...
            book.add_user_order(tokens[si], tokens[bi], offered, amount);
        }
...
fn many_small_vs_one_large() {
    ...
    book.add_user_order(eth(), usdc(), 100, 160000);

    // 20 small USDC→ETH orders
    for _ in 0..20 {
        book.add_user_order(usdc(), eth(), 10000, 5);
    }
...
fn all_same_direction() {
    ...
    for i in 1..=10 {
        book.add_user_order(eth(), usdc(), i * 10, i * 16000);
    }
...
fn barely_profitable_direct() {
    ...
    book.add_user_order(usdc(), eth(), 101, 100);
...
fn exactly_at_oracle_no_match() {
    ...
    book.add_user_order(eth(), usdc(), 10, 20000);
...
fn triangle_one_unit_bottleneck() {
    ...
    let mut book = OrderBook::new(feed);
    book.add_user_order(eth(), usdc(), 10000, 9000);
    book.add_user_order(usdc(), sol(), 10000, 9000);
    // Tiny bottleneck: 2 SOL for 1 ETH
    book.add_user_order(sol(), eth(), 2, 1);
...
fn triangle_rounding_to_zero() {
    ...
    book.add_user_order(usdc(), sol(), 300, 1);
    ...
    book.add_user_order(sol(), eth(), 20, 1);
...
fn phase1_steals_triangle_leg() {
    ...
    book.add_user_order(usdc(), eth(), 16000, 10);

    // These would form a triangle with ETH→USDC, but phase 1 eats ETH→USDC first
    book.add_user_order(usdc(), sol(), 16000, 80);
    book.add_user_order(sol(), eth(), 80, 5);
    ...
    for trial in 0..100 {
        ...
        for _ in 0..num_triangles {
            ...
            for &(si, di) in &[(ai, bi), (bi, ci), (ci, ai)] {
                ...
                if requested == 0 { continue; }
                book.add_user_order(tokens[si], tokens[di], offered, requested);
            }
        }
...
fn conservation_detailed_direct_match() {
    ...
    book.add_user_order(eth(), usdc(), 10, 16000);    // generous
    book.add_user_order(eth(), usdc(), 5, 9000);      // generous
    book.add_user_order(usdc(), eth(), 18000, 10);     // generous
    book.add_user_order(usdc(), eth(), 10000, 5);      // generous
...
fn conservation_detailed_triangle() {
    ...
    book.add_user_order(eth(), usdc(), 200, 150);
    book.add_user_order(usdc(), sol(), 200, 150);
    book.add_user_order(sol(), eth(), 200, 150);
...
fn order_book_consistency_after_run() {
    ...
    for trial in 0..50 {
        ...
        // Verify active count
        let actual_active = engine.book.orders.iter().filter(|o| o.is_active()).count() as u32;
        assert_eq!(
            actual_active,
...
fn stress_50_orders_per_direction() {
    ...
    for _ in 0..50 {
        ...
        let requested = (offered as u128 * 200_000u128 * rate_pct as u128
            / (100u128 * 100)) as u32;
        if requested > 0 {
            book.add_user_order(eth(), usdc(), offered, requested);
        }
    }
    ...
    for _ in 0..50 {
        ...
        if requested > 0 {
            book.add_user_order(usdc(), eth(), offered, requested);
        }
    }
...
fn offered_for_precision_boundaries() {
    ...
    let order_big_requested = Order {
        id: 1, offered_token: eth(), requested_token: usdc(),
    ...
    let order_equal = Order {
        id: 2, offered_token: eth(), requested_token: usdc(),
...
fn deterministic_output() {
    ...
    for _ in 0..20 {
        ...
        for &(si, bi, offered, requested) in &orders {
            book1.add_user_order(tokens[si], tokens[bi], offered, requested);
        }
        ...
        for &(si, bi, offered, requested) in &orders {
            book2.add_user_order(tokens[si], tokens[bi], offered, requested);
        }
        ...
        for i in 0..engine1.book.orders.len() {
            assert_eq!(
                engine1.book.orders[i].requested_remaining,
                engine2.book.orders[i].requested_remaining,
                "order {} fill differs between runs", i
            );
        }
    ...
    for trial in 0..200 {
        ...
        for _ in 0..n {
            ...
            let requested = (offered as u128 * prices[si] as u128 * rate_pct as u128
                / (prices[bi] as u128 * 100)) as u32;
            if requested == 0 { continue; }
            if let Some(id) = book.add_user_order(tokens[si], tokens[bi], offered, requested) {
        ...
        for (ti, &token) in tokens.iter().enumerate() {
            ...
            for &(id, off_tok, req_tok, _offered, _requested) in &order_snapshot {
                let order = &engine.book.orders[id as usize];
                let filled = order.requested_filled();
...
fn same_token_order_rejected() {
    ...
    let result = book.add_user_order(eth(), eth(), 100, 50);
...
fn phase1_greedy_vs_phase2_triangle_tradeoff() {
    ...
    book.add_user_order(usdc(), eth(), 18000, 10);
...
fn order_filled_by_both_phases() {
    ...
    let shared_id = book.add_user_order(eth(), usdc(), 50, 80000).unwrap();

    // Phase 1: small USDC→ETH direct match
    book.add_user_order(usdc(), eth(), 16000, 10);

    // Phase 2: triangle using remaining ETH→USDC
    book.add_user_order(usdc(), sol(), 64000, 320);
    book.add_user_order(sol(), eth(), 320, 20);

    let mut engine = MatchingEngine::new(book);
    let batch = engine.run();

    let shared_order = &engine.book.orders[shared_id as usize];
...
fn identical_orders_fair_matching() {
    ...
    let id1 = book.add_user_order(eth(), usdc(), 10, 16000).unwrap();
    let id2 = book.add_user_order(eth(), usdc(), 10, 16000).unwrap();
    ...
    let order1 = &engine.book.orders[id1 as usize];
    let order2 = &engine.book.orders[id2 as usize];
...
fn no_phantom_fills() {
    ...
    book.add_user_order(eth(), usdc(), 1, 100000);
    book.add_user_order(usdc(), eth(), 1, 100000);
...
fn settlement_batch_completeness() {
    ...
    for trial in 0..50 {
        ...
        let total_orders = book.orders.len();
        let mut engine = MatchingEngine::new(book);
        let batch = engine.run();

        // Every filled order ID must be a valid index
        for &oid in &batch.filled_orders {
            assert!((oid as usize) < total_orders, "invalid order ID in filled set");
        }
...
fn both_sides_one_unit() {
    ...
    let mut book = OrderBook::new(feed);
    book.add_user_order(eth(), usdc(), 2, 1);
    book.add_user_order(usdc(), eth(), 2, 1);
...
fn deep_order_book_100_levels() {
    ...
    for i in 0..100 {
        ...
        book.add_user_order(usdc(), eth(), usdc_amount, 1);
    }
